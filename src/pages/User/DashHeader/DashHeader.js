...
const DashHeader = (props) => {
    ...
    const goToPreview = (data) => {
        ...
        navigate('/user/preview', { state: { id: data?.receiptId } });
    };
    useEffect(() => {
        const items = JSON.parse(localStorage.getItem('user'));
        if (items) {
            setPic(items.profileImage);
            // setUser_id(items._id);
        }
    ...
    socketIo.on('EmitNotification', (data) => {
        ...
        <Box className={styles.wrapper}>
            <Box className={styles.mainHeader}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Box onClick={Toggel1} className={styles.roundWrapper} sx={{ cursor: 'pointer', display: { xs: 'none', sm: 'block' } }}>
                        {props.data.UserProfileImageUrl ? (
                            <img src={props.data.UserProfileImageUrl} alt="user" />
                        ) : pic ? (
            ...
            >
                <MenuItem onClick={handleClose} component={Link} to="/user/profile">
                    <Box sx={{ display: 'flex', alignItems: 'center', pt: 1.5, pl: 1.2 }}>
                        ...
                        <Typography sx={{ paddingLeft: '7px', fontWeight: '400', fontFamily: 'poppins', color: '#344054' }} variant="h5">
                            Profile
                        </Typography>
                    </Box>
                </MenuItem>
                <MenuItem onClick={handleClose} component={Link} to="/user/createReciept">
            ...
            >
                <MenuItem onClick={handleClose} sx={{ p: 0 }} component={Link} to="/user/notifications">
                    <Box
                        sx={{
                            display: 'flex',
                    ...
                    >
                        <Typography sx={{ paddingLeft: '16px', fontWeight: '400', fontFamily: 'poppins', color: '#344054' }} variant="h5">
                            Notifications
                        </Typography>
                        <Typography sx={{ paddingRight: '20px', fontWeight: '400', fontFamily: 'poppins', color: '#344054' }} variant="h5">
                            View All
                        </Typography>
                    </Box>
                </MenuItem>
